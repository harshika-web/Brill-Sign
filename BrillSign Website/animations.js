document.addEventListener('DOMContentLoaded', () => {
    ...
    const scenarioBackgrounds = [
        ...
        'https://images.unsplash.com/photo-1531297484001-80022131f5a1?auto=format&fit=crop&q=80&w=1600'  // Exit Safe / Storage
    ];
    ...
    const handleScenarioScroll = () => {
        ...
        if (bgOverlay) {
            ...
            if (!currentImg.includes(targetImgUrl)) {
                // Apply a professional dark gradient overlay to keep it subtle
                bgOverlay.style.backgroundImage = `linear-gradient(to right, rgba(15, 23, 42, 0.9), rgba(15, 23, 42, 0.6)), url("${targetImgUrl}")`;
            }
        }
    };
    ...
    if (heroIcons.length > 0 && integrationGrid && floatingIconsContainer) {
        ...
        const handleIconAssembly = () => {
            ...
            // Assembly progress based on scroll distance
            // Since hero is sticky, we use the initial scroll to drive animation.
            // 300px corresponds to a "little scroll" as requested.
            const maxScroll = 300;
            const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
            const scrollLeft = window.pageXOffset || document.documentElement.scrollLeft;
            ...
            if (heroContent) {
                // Move up by 100px when fully scrolled
    ...
    const updateStrategicDisplay = (index) => {
        ...
        const orbitRing = document.querySelector('.expanded-orbit .orbit-3');
        if (orbitRing) {
            orbitRing.style.transform = `translate(-50%, -50%) rotate(${index * 90}deg)`;
        }
    };
